...
# 203 Server (Source: MiniLms..mst_center)
SOURCE_CONN_STR = (
...
# 164 Server (Destination: sonata_satark..Mst_Center_Geo)
DEST_CONN_STR = (
    "DRIVER={ODBC Driver 17 for SQL Server};"
...
SOURCE_QUERY = "SELECT * FROM MiniLms..mst_center"
DEST_TABLE = "Mst_Center_Geo"
...
log_filename  = os.path.join(LOG_FOLDER, f"Center_Geo_Transfer_Log_{timestamp_str}.log")
...
def verify_table_exists(connection_string, table_name):
    try:
        with pyodbc.connect(connection_string) as conn:
            cursor = conn.cursor()
            cursor.execute(
        ...
        logging.error(f"Error verifying table {table_name}: {e}")
        return False
...
# ──────────────────────────────────────────────────────────────
#  DELETE REFRESH LOGIC (Delete coming centers before insert)
# ──────────────────────────────────────────────────────────────
def delete_coming_centers(dest_conn_str, dest_table, full_df):
    """
    Deletes matching incoming center records from destination table (164)
    before inserting the refreshed data.
    """
    possible_id_cols = ['CenterID', 'center_id', 'Center_ID', 'Center_Id', 'CENTERID', 'CenterCode', 'ID', 'id']
    id_col = None
    ...
    with pyodbc.connect(dest_conn_str) as conn:
        ...
        if id_col:
            ...
            logging.info(f"Found center ID column '{id_col}' with {len(unique_ids):,} unique IDs in source batch.")
            if unique_ids:
                # Delete in batches of 1,000 to avoid SQL Server parameter limits
                batch_size = 1000
                deleted_count = 0
                ...
                conn.commit()
                logging.info(f"Successfully deleted {deleted_count:,} existing matching center rows from destination [{dest_table}].")
        else:
            logging.warning(f"No specific Center ID column detected. Refreshing entire table [{dest_table}]...")
...
def _worker_insert_chunk(args):
    """
    Worker function executed in thread pool. Each thread opens its own pyodbc connection.
    """
...
def run_center_data_refresh():
    ...
    logging.info("=" * 60)
    logging.info("SARTHI DATA REFRESH: MiniLms (203) -> sonata_satark (164)")
    ...
    # Refresh step: Delete existing coming centers from 164
    logging.info(f"Deleting existing records for coming centers in destination table [{DEST_TABLE}]...")
    delete_coming_centers(DEST_CONN_STR, DEST_TABLE, df_source)

    # Slice data into chunks for multithreaded insertion
    chunks = [df_source[i:i + CHUNK_SIZE] for i in range(0, len(df_source), CHUNK_SIZE)]
    _total_chunks = len(chunks)

    logging.info(f"Inserting data into 164 using {MAX_WORKERS} parallel threads across {_total_chunks} chunk(s)...")
