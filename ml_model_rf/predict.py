...
from .config import MODEL_FILE_PATH, CLASSIFIER_FILE_PATH, LABEL_ENCODER_PATH, CATEGORICAL_ENCODER_PATH, FEATURE_COLS, score_to_grade
...
def get_models():
    ...
    if _MODEL_CACHE is None:
        ...
        cat_encoder = joblib.load(CATEGORICAL_ENCODER_PATH)
        
        _MODEL_CACHE = (regressor, classifier, label_encoder, cat_encoder)
...
def predict_branch_audit_score(branch_data: Union[Dict[str, Any], pd.DataFrame]) -> Dict[str, Any]:
    ...
    regressor, classifier, label_encoder, cat_encoder = get_models()
    ...
    for idx, score in enumerate(clipped_scores):
        ...
        # 🌟 Secret Weapon: The Regressor is mathematically more accurate than the Classifier!
        # So instead of using classifier.predict(), we just map the regressor's score directly to a Grade!
        mapped_grade = score_to_grade(score_val)
        ...
        print(f"  [PREDICTION RESULT] Regressor Score: {score_val:.2f}/100 (Grade: {mapped_grade})")
        if warnings:
            print(f"  [RISK WARNINGS] {', '.join(warnings)}")

        results.append({
            'predicted_score': score_val,
            'predicted_grade': mapped_grade,
            'risk_level': 'HIGH' if mapped_grade in ['C', 'D'] else 'NORMAL',
            'warnings': warnings,
            'model_used': 'RandomForestRegressor_Mapped'
        })
