...
def extract_stored_procedures(engine):
    ...
    try:
        ...
        print(f"[OK] Extracted {len(procs)} user-defined stored procedures from database.")
    except Exception as e:
        print(f"Warning extracting stored procedures: {e}")
    return procs
...
def build_sp_dataset():
    ...
    print(f"[SUCCESS] Stored procedure dataset extraction complete! Saved {len(dataset)} pairs to '{out_file}'.")
    return dataset
