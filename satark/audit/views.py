...
@csrf_exempt
def get_branch_same_mobile_data(request):
    data, error_resp = parse_post_payload(request, "get_branch_same_mobile_data")
    if error_resp: return error_resp
    user, error_resp = validate_user_view(data.get('token', ''))
    if error_resp: return error_resp
    ...
    as_on_date = data.get('as_on_date')
    report_type = data.get('report_type', 'Same-Mobile-Customer')
    if not branch_id:
        return JsonResponse({'success': False, 'message': 'branch_id or branch_name is required'}, status=400)

    try:
        query = GetBranchSameMobileDataQuery(branch_id=str(branch_id), branch_name=data.get('branch_name'), as_on_date=as_on_date, report_type=report_type)
        result = dispatcher.query(query)
        return JsonResponse(result)
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Internal Server Error: {str(e)}'}, status=500)
...
@csrf_exempt
def get_branch_same_aadhaar_data(request):
    data, error_resp = parse_post_payload(request, "get_branch_same_aadhaar_data")
    if error_resp: return error_resp
    user, error_resp = validate_user_view(data.get('token', ''))
    if error_resp: return error_resp
    ...
    branch_id = data.get('branch_id') or data.get('branch_name')
    as_on_date = data.get('as_on_date')
    report_type = data.get('report_type', 'Same-Aadhar-Customer')
    if not branch_id:
        return JsonResponse({'success': False, 'message': 'branch_id or branch_name is required'}, status=400)
...
@csrf_exempt
def get_branch_same_coapplicant_data(request):
    data, error_resp = parse_post_payload(request, "get_branch_same_coapplicant_data")
    if error_resp: return error_resp
    user, error_resp = validate_user_view(data.get('token', ''))
    if error_resp: return error_resp
    ...
    branch_id = data.get('branch_id') or data.get('branch_name')
    as_on_date = data.get('as_on_date')
    report_type = data.get('report_type', 'Same-CoApplicant-Customer')
    if not branch_id:
        return JsonResponse({'success': False, 'message': 'branch_id or branch_name is required'}, status=400)

    try:
        query = GetBranchSameCoApplicantDataQuery(branch_id=str(branch_id), branch_name=data.get('branch_name'), as_on_date=as_on_date, report_type=report_type)
        result = dispatcher.query(query)
        return JsonResponse(result)
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Internal Server Error: {str(e)}'}, status=500)
...
@csrf_exempt
def get_branch_same_applicant_coapplicant_data(request):
    data, error_resp = parse_post_payload(request, "get_branch_same_applicant_coapplicant_data")
    if error_resp: return error_resp
    user, error_resp = validate_user_view(data.get('token', ''))
    if error_resp: return error_resp
    ...
    branch_id = data.get('branch_id') or data.get('branch_name')
    as_on_date = data.get('as_on_date')
    report_type = data.get('report_type', 'Same-Applicant-CoApplicant')
    if not branch_id:
        return JsonResponse({'success': False, 'message': 'branch_id or branch_name is required'}, status=400)
